# Curriculum

This curriculum is designed as a six-module teaching track for technical product managers and platform leaders.

## Learning Outcomes

By the end, learners should be able to:

- Describe APIs as products with users, jobs to be done, onboarding journeys, and lifecycle states.
- Read system design diagrams and identify product-relevant trade-offs.
- Explain OAuth, scopes, API keys, tenants, and consent using product language.
- Design API versioning and deprecation plans.
- Review OpenAPI specifications for usability and governance.
- Define platform KPIs that connect technical health to business outcomes.

## Format

Each module includes:

- Key concepts.
- Architecture or product diagram.
- Practical exercise.
- Discussion questions.
- Interview-style prompts.

